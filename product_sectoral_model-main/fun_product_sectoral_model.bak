...
/*
Product Variable Cost. Depends on the firm wage and average productivty, labour cost (the same for all products of the firm) and the input cost, which is different for each product.
*/
v[1]=V("F_Wage");
...
*/
	...
	v[4]=V("quality_adjustment");		    		//adjustment parameter							
...
//EQUATION("F_Product_RND_Expenses")
/*
Firm product rnd expenses. Share of total aftertax revenue allocated to product rnd.
*/
//v[1]=SUM("P_Aftertax_Revenue");
//v[2]=V("F_product_rnd_proportion");     // to be endogenised?
//v[3]=v[1]*v[2]; 
//RESULT(v[3])
...
EQUATION("P_Product_RND_Share")
	...
	v[4]=V("product_adjustment");		    		//adjustment parameter							
...
*/
	...
	v[1]=V("P_Desired_Market_Share");                //desired market-share 
	v[2]=VL("P_Avg_Market_Share",1);                 //firm average market share in the last period   
...
v[3]=V("F_Product_RND_Expenses");							  												
v[4]=V("F_Financial_Obligations");
v[5]=WHTAVE( "P_Effective_Production", "P_Variable_Cost" );
...
*/
//v[0]=V("F_Product_RND_Expenses"); 
v[0]=V("F_Product_Innovation_RND_Expenses");          
//v[1]=V("F_Wage"); 		
//v[2]=V("P_Innovation_RND_Share");		
//v[3]=(v[0]/v[1])*(v[2]); 
v[4]=V("S_product_difficulty");                   
v[5]=1-exp(-v[0]*v[4]);                   		
if(RND<v[5])  
	{
...
//v[1]=V("F_Wage"); 		
//v[2]=V("F_product_innovation_share");		
//v[3]=(v[0]/v[1])*(v[2]); 
v[4]=V("S_product_patent");                   
v[5]=1-exp(-v[0]);                   		
if(RND<v[5])  
	{
	...
	if(v[11]!=0)                                                        //if the cycle counter is different than zero (a product was found)                              
		...
		if(t>v[13]+v[4])                                                //if the time period is geater than the birth date + patent period
			{			 
...
v[1]= VL("M_Effective_Orders",1);
v[2]=V("M_max_demand");
v[3]=V("max_entry_number");
v[5]= ((v[2] - v[1])/v[1]);
v[6]= round(v[5]);
...
*/
v[0]=V("M_Number_Potential_Entrants");									
...
CYCLES(PARENT, cur, "FIRM")
     ...
     CYCLES(cur, cur1, "PRODUCT")
     	{
     	v[1]=VS(cur1, "P_Competitiveness");	
     	
cur2 = SEARCH_CND(v[1],"M_Avg_Competitiveness")
v[8]=VS(cur2, "P_id");
cur3=SEARCH_CNDS(PARENT, "P_id", v[8]);
			for (i=0; i<=v[0];i++)
			{
				v[2]=VL("M_Effective_Orders",1);
				v[3]=VL("M_Effective_Orders",2);
				v[4]=((v[2]-v[3])/v[3]);
				v[5]=V("M_entry_barrier");
				v[6]=(v[5]*v[4]);
				v[7]=bernoulli(v[6]);
				v[9]=COUNT("FIRM");
				if (RND<v[7]) 
 				cur4=ADDOBJ_EX("FIRM",cur3);
 				WRITES(cur4,"F_id", V[9]+1),	
 				WRITES(cur4,"F_date_birth",t);
 				
        	}}}       
RESULT(0)     
     
EQUATION("M_Firm_Exit")
/*
Firm exit process from each market
*/	
RESULT(0)

EQUATION("S_Firm_Exit")
/*
Firm exit process from the whole industry
*/
RESULT(0)
